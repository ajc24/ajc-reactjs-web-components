 ...
 */
const ScrollMenuBarItemsRight = props => {
  ...
  const getContainerDOMElement = () => {
    return document.querySelector(`div[id="${props.id}--container--scroll-menu-bar-items-right"]`);
  };
  ...
  return (
    <div className={containerCss} id={`${props.id}--container--scroll-menu-bar-items-right`}>
      <button aria-hidden={`${props.isHidden}`} aria-label="Scroll the list of menu bar items to the right." className={buttonCss}
        id={`${props.id}--scroll-menu-bar-items-right`} onClick={handleOnClick} onKeyDown={handleOnKeyDown}
        tabIndex={props.isHidden === true ? '-1' : '0'} type="button" dangerouslySetInnerHTML={{ __html: 'Next<br />&rarr;' }} />
    </div>
  );
}
